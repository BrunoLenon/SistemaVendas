...
def relatorio_campanhas():
    ...
    with SessionLocal() as db:
        ...
        for emp in emps_scope:
            ...
            # Cache para detalhar combos (parcial por item) sem repetir consultas dentro da mesma EMP
            ...
            def _combo_details_for_vendor(combo_id: int, vend: str):
                        ...
                        for it_ in itens_:
                            qtd_maps.append(
                                _calc_qtd_por_vendedor_para_combo_item(
                                    db, emp, it_, combo.marca, periodo_ini, periodo_fim
                                )
                            )
                ...
                for it_, qmap in zip(itens_, qtd_maps):
                    ...
                    unit = (
                        it_.valor_unitario
                        if it_.valor_unitario is not None
                        else (combo.valor_unitario_global if combo else None)
                    )
                    unit = float(unit or 0.0)
