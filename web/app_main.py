...
def _current_branding(db) -> dict:
    """Retorna branding atual (tema sazonal ativo ou padrão) com fallback seguro."""
    default_logo = _get_setting(db, "branding.default_logo_url")
    default_favicon = _get_setting(db, "branding.default_favicon_url")
    default_version = _get_setting(db, "branding.default_version", "")
    default_login_left = _get_setting(db, "branding.login_logo_left_url", default_logo)
    default_login_right = _get_setting(db, "branding.login_logo_right_url", default_logo)
            ...
            try:
                ...
                def _read_file(f, max_bytes: int, allowed_ext: set[str]):
                    ...
                    if len(data) > max_bytes:
                        raise ValueError("Arquivo muito grande. Limite: 6MB para logos e 400KB para favicon.")
                    ctype = f.mimetype or mimetypes.guess_type(filename)[0] or "application/octet-stream"
                    return filename, data, ctype

                logo = _read_file(logo_file, max_bytes=6_000_000, allowed_ext={'.png', '.jpg', '.jpeg', '.webp', '.svg'})
            ...
            try:
                ...
                if logo_file and logo_file.filename:
                    ...
                    if len(data) > 6_000_000:
                        raise ValueError("Logo do tema muito grande. Limite: 6MB.")
...
def _dados_admin_geral(mes: int, ano: int, emp_scope: list[str] | None = None):
    ...
    with SessionLocal() as db:
        ...
        emp_rows = [
            {"emp": (emp or "").strip(), "valor_bruto": float(bruto or 0.0), "valor_devolvido": float(devol or 0.0), "valor_atual": float(liquido or 0.0)}
            for emp, bruto, devol, liquido in (
                ...
                ).group_by(DashboardCache.emp).order_by(func.coalesce(func.sum(DashboardCache.valor_liquido), 0.0).desc()).limit(30).all()
            )
        ]
        vend_rows = [
            {"vendedor": (v or "").strip().upper(), "valor": float(val or 0.0)}
            for v, val in (
                q.with_entities(DashboardCache.vendedor, func.coalesce(func.sum(DashboardCache.valor_liquido), 0.0).label("valor"))
                .group_by(DashboardCache.vendedor).order_by(func.coalesce(func.sum(DashboardCache.valor_liquido), 0.0).desc()).limit(30).all()
            )
        ]
